...
def parse_args(extra_args_provider=None, defaults={},
    ...
    if args.fid_training:
        for req_arg in ['pretrained_t5_load']:
            _check_arg_is_not_none(args, req_arg)
...
def _add_distributed_args(parser):
    ...
    group.add_argument('--use-slurm', action='store_true',
                       help='If set, use the SLURM env variables')
    return parser
...
def _add_data_args(parser):
    ...
    group.add_argument('--seq-length-ret', type=int, default=256,
...
def _add_emdr2_args(parser):
    ...
    group.add_argument('--fid-training', action='store_true',
                       help="Whether training FiD model")
    ...
    group.add_argument('--index-reload-interval', type=int, default=500,
                       help='how often (iterations) to refresh MIPS index during EMDR2 training')
