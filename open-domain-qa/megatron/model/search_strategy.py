...
class SampleOrGreedySearch(object):
    ...
    def generate_output(self, model, query_uid, query_ids_bert, query_types, query_mask_bert, ctx_ids, query_ids_t5, query_ids_t5_len):
        batch, x_length = query_ids_bert.shape
        ...
        for i in range(self.max_decode_length):
            logits, all_query_context_hidden_states, all_query_context_ids_unflat = \
                model(query_uid,
                      query_ids_bert,
                      query_types,
                      query_mask_bert,
